...
// setResourceTags augments the Index spec with the resource tags, which are
// not returned by GetIndex. They have a dedicated ListTagsForResource API.
// Reading them here keeps observed state consistent with desired state and
// prevents spurious deltas on every reconciliation.
...
// encryption configuration even when the user did not specify one, so GetIndex
// always returns a value. If the user did not specify it in their spec we adopt
// the observed default to avoid flagging a difference for the server-managed
// (and immutable) value.
