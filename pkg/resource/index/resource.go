...
func (r *resource) SetIdentifiers(identifier *ackv1alpha1.AWSIdentifiers) error {
	...
	r.ko.Spec.IndexName = &identifier.NameOrID

	f1, f1ok := identifier.AdditionalKeys["vectorBucketName"]
	if f1ok {
		r.ko.Spec.VectorBucketName = aws.String(f1)
	}
...
func (r *resource) PopulateResourceFromAnnotation(fields map[string]string) error {
	primaryKey, ok := fields["indexName"]
	if !ok {
		return ackerrors.NewTerminalError(fmt.Errorf("required field missing: indexName"))
	}
	r.ko.Spec.IndexName = &primaryKey

	f1, f1ok := fields["vectorBucketName"]
	if f1ok {
		r.ko.Spec.VectorBucketName = aws.String(f1)
	}
