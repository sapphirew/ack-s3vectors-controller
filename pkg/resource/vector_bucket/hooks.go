...
// setResourceTags augments the VectorBucket spec with the resource tags, which
// are not returned by GetVectorBucket. They have a dedicated
// ListTagsForResource API. Reading them here keeps observed state consistent
// with desired state and prevents spurious deltas on every reconciliation.
...
// customPreCompare normalizes the nil-able encryption configuration before the
// generated delta comparison runs. S3 Vectors always applies a default
// encryption configuration even when the user did not specify one, so
// GetVectorBucket always returns a value. If the user did not specify it in
// their spec we adopt the observed default to avoid flagging a difference for
// the server-managed (and immutable) value.
